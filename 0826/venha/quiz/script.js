...
function add() {
    ...
    function addQuestion() {
            ...
            if (!answered) {
                ...
                for (i = 0; i < buttons.length; i++) {
                    if (buttons[i].answer === a) {
                        ...
                        e.className = "QA";
                        c.appendChild(e);
                        if (buttons[i] === this) { e.innerHTML = "CORRECT"; e.style.color = "#080"; } else { e.innerHTML = "INCORRECT"; e.style.color = "#f00"; }
                    } else
                        if (buttons[i] === this) { buttons[i].style.color = "#f00"; } else { buttons[i].style.color = "#888"; }
                }
        ...
        for (i = 0; i < 4; i++) {
            ...
            e.innerHTML = e.answer = (i ? (n1 + (n2 + (Math.random() > 0.5 || i >= n2 ? i : -i))) : (n1 + n2));
            e.onclick = checkAnswer;
            buttons[Math.random() > 0.5 ? "push" : "unshift"](e);
        }
        for (i = 0; i < buttons.length; i++) { c.appendChild(buttons[i]); }
    }
...
function div() {
    ...
    function divQuestion() {
        ...
        function checkAnswer() {
            let a = Math.round((n1 / n2) * 100) / 100, i, e;
            if (!answered) {
                answered = true;
                for (i = 0; i < buttons.length; i++) {
                    if (buttons[i].answer === a) {
        ...
        for (i = 0; i < 4; i++) {
            e = document.createElement("button");
            e.innerHTML = e.answer = Math.round((i ? (n1 / (n2 + (Math.random() > 0.5 || i >= n2 ? i : -i))) : (n1 / n2)) * 100) / 100;
            e.onclick = checkAnswer;
            buttons[Math.random() > 0.5 ? "push" : "unshift"](e);
        }
        for (i = 0; i < buttons.length; i++) { c.appendChild(buttons[i]); }
